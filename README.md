...
-   **IonicEfus.jl**: The foundation of the ecosystem. It provides the core declarative UI templating language (Efus), a powerful reactivity model, and a component-based architecture. For a deep dive into reactivity, components, and the Efus language, please refer to the [IonicEfus.jl documentation](https://github.com/ken-morel/IonicEfus.jl).
...
-   `spa()`: A convenience function for creating a Single-Page Application. It creates an application and a single window in one call.

    ```julia
    app = spa(id="com.example.spa") do _, _
        # ... return a page
    end
    run(app)
    ```
...
Pages represent the content of a window. They can be created in several ways:
...
-   **Macros**: For quick and easy page creation using the Efus templating language.
    ...
    -   `@reloadablepage_str`: Creates a `ReloadablePage` that can be hot-reloaded.
...
2.  **Marking as Dirty**: When you change a reactive property (e.g., `my_reactant' = new_value`), the component that depends on it is marked as "dirty" by calling `dirty!(component, :property_name)`.
3.  **Scheduling**: This `dirty!` call automatically pushes a `ComponentUpdate` task onto the window's dedicated `Scheduler` (provided by `Atak.jl`). You can optionally specify a priority (`High`, `Normal`, `Low`) for the update.
4.  **Updating**: The `Scheduler` runs on a background thread, processing the update queue. It then executes the `update!(component)` method on the main GTK thread, which efficiently updates only the necessary parts of the GTK widget.

This architecture ensures that your UI remains fluid and responsive, as expensive computations don't block the main UI thread.
...
2.  **Define your pages as `ReloadablePage`s**:
    ...
    ```
    By defining your page's content in a function and passing that function to `ReloadablePage`, `Revise.jl` can update the function's definition, and `Gtak.reload!` can rebuild the page with the new content.
